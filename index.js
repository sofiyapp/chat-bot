...
// Обновление даты последнего сообщения при любом сообщении
bot.on('message', (msg) => {
  ...
  db.query(
    ...
    [userId, today, today],
    (err) => {
      if (err) console.error('Ошибка при обновлении lastMessage:', err);
    }
  );
});
...
// Ежедневная проверка в 13:00 по мск
setIntervalAsync(() => {
  const now = moment().tz('Europe/Moscow');
  if (now.hour() === 13 && now.minute() < 10) {
    const threshold = moment().subtract(2, 'days').format('YYYY-MM-DD');
    db.query('SELECT * FROM Users WHERE lastMessage < ?', [threshold], (err, users) => {
      if (err) return console.error('Ошибка при получении пользователей:', err);
      users.forEach(user => {
        db.query('SELECT * FROM items ORDER BY RAND() LIMIT 1', (err, results) => {
          if (err || results.length === 0) {
            console.error('Ошибка при выборе случайного предмета:', err);
            return;
          }
          const item = results[0];
          bot.sendMessage(user.ID, `(${item.id}) - ${item.name}: ${item.desc}`);
        });
      });
    });
  }
